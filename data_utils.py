...
def specific_split(own_label,other_labels_and_ratios,data,size,own_label_ratio,data_mask):
    ...
    for index,(i,l,m) in enumerate(zip(data[0],data[1],data_mask)):
        if l == own_label and m != 0:
    ...
    return data_mask,client_data,client_label

def create_others_split_ratio(selected_class,num_of_class):
    return {i:(100/(num_of_class-1))/100 if i!=selected_class else 0 for i in range(num_of_class)}
...
def specific_split_data(images,labels,num_clients,shuffle,name_dataset,dim,sel_label,other_labels,data_size=None,specific_ratio=0.25):
    ...
    Data_Split_Dict = {} # Client_name: (image,label)
    data_mask = torch.ones(len(labels_tensor))
    for index,name in enumerate(client_list):
        if name[-1] != str(sel_label):
            ...
            data_mask,s_data,s_label = specific_split(own_label=sel_label,other_labels_and_ratios=other_labels,data=[images_normalized,labels_tensor],
                                                 size=len_data_per_clients,own_label_ratio=specific_ratio,data_mask=data_mask)
...
def IID_split_data(images,labels,num_clients,shuffle,name_dataset,dim,data_size=None,specific_ratio=0.25):
    ...
    data_size = len_data_per_clients = len(images)//num_clients if data_size==None else data_size
        
    Data_Split_Dict = {} # Client_name: (image,label)
    data_mask = torch.ones(len(labels))
    for index,name in enumerate(client_list):
        sel_label = int(name[-1])
        other_labels = create_others_split_ratio(selected_class=sel_label,num_of_class=10)
        data_mask,s_data,s_label = specific_split(own_label=sel_label,other_labels_and_ratios=other_labels,
                                                             data=[images_normalized,labels_tensor],size=len_data_per_clients,
                                                             own_label_ratio=specific_ratio,data_mask=data_mask)
        Data_Split_Dict[name] = [s_data,s_label]
