...
class Env(Structure):
    ...
    _fields_ = [
        ...
        ('packetDroppedFlag', c_bool),
        ('clientToBeDiscarded',c_int),
...
class Act(Structure):
    ...
    _fields_ = [
        ('accuracy', c_double),
        ('server_accuracy', c_double),
        ('clientUpdateDuration',c_double)
    ]
...
exp = Experiment(mempool_key, mem_size, 'MASTER', '../../')
...
try:
    ns3Settings["m_client"] = len(client_list)
    for round in range(ROUNDS):
        exp.reset()
        ns3Settings["SeedRunNumber"] = 100*round+1
        ...
        while not fl.isFinish():
            with fl as data:
                ...
                """
                if data.env.packetDroppedFlag:
                    pass
                """

                if data.env.clientUpdateFlag:
                    selectedClient = data.env.clientId
                    participating_clients.append(all_clients[selectedClient])
                    #print("PYTHON :: Client Name:{}".format(all_clients[selectedClient].name))
                    
                    all_clients[selectedClient].syncronize_with_server_voter(server)
                    client_update_start = time.time()
                    loss,val_accuracy= all_clients[selectedClient].compute_weight_update(epochs = LOCAL_EPOCHS, 
                    ...
                    print("PYTHON :: ClientName:{}\tAccuracies:{}".format(all_clients[selectedClient].name,val_accuracy))
                    ...
                    data.act.clientUpdateDuration = client_update_duration
                    data.act.accuracy = val_accuracy#["accuracy"]
                    
                    client_record = {}
                    client_record["client_id"] = all_clients[selectedClient].id_num
                    ...
                    client_record["data_size"] = len(data_dict_split[all_clients[selectedClient].name][1])
                    client_record["batch_size"] = BATCH_SIZE
                    client_record["local_epoch"] = LOCAL_EPOCHS 

                    clients[all_clients[selectedClient].name] = client_record
                ...
                if data.env.isRoundFinished:
                    ...
                    json.dump(RECORDS, open( "HISTORICAL_RECORDS_STUDY_SCORED_CL_COL_CL"+str(CORRUPTED_CLIENTS_NUM)+".json", 'w' ))
